...
export function POSKeypad({ onKeyPress, mode }: POSKeypadProps) {
  return (
    <div className="w-full flex flex-col bg-black px-4 sm:px-6 pb-6 gap-3 shrink">
      ...
      <div
        className={cn(
          "grid grid-cols-4 gap-2 sm:gap-4 lg:gap-4 justify-items-center max-w-[320px] lg:max-w-[400px] mx-auto w-full transition-all shrink-0",
          ...
          row.map((k) => (
            <button
              ...
              className={cn(
                ...
                "w-[60px] h-[60px] sm:w-[72px] sm:h-[72px] lg:w-[80px] lg:h-[80px]",
                k.type === "num"
                  ? "bg-[#1e1e1e] text-[#eeeeee] text-2xl sm:text-3xl lg:text-3xl active:bg-[#2c2c2c]"
                  : k.type === "op"
                    ? "bg-[#1e1e1e] text-[#3cc366] text-2xl sm:text-3xl lg:text-3xl active:bg-[#2c2c2c]"
                    : k.type === "ac"
                      ? "bg-[#1e1e1e] text-[#ff4c4c] text-2xl sm:text-3xl lg:text-3xl active:bg-[#2c2c2c]"
                      : k.type === "eq"
                        ? "bg-[#3cc366] text-black text-3xl sm:text-4xl lg:text-4xl pb-1 active:bg-[#32a454]"
                        : "bg-[#1e1e1e] text-white",
                k.label === "⌫" && "text-xl sm:text-2xl lg:text-2xl",
                k.label === "00" && "text-xl sm:text-2xl lg:text-2xl",
              )}
            >
              {k.label}
            </button>
